...
/**
 * 계층형 토폴로지 레이아웃 상수 (SVG viewBox 좌표계).
 * 서버(최상단) → 1단 사일로(한 줄) → 하위 노드(부모 아래 세로 스택)의 톱다운 트리.
 * viewBox 크기는 노드 구성에 따라 `lib/topology.ts`가 동적으로 계산한다.
 */
export const TOPOLOGY = {
  ...
  /** 1단 사일로 y 좌표 */
  siloY: 290,
  /** 1단 사일로 가로 간격 — 노드명 라벨과 저장소 3개 열이 겹치지 않는 폭 */
  siloGapX: 92,
  /** 좌우 여백 */
  marginX: 22,
  /** 하위 노드 세로 간격 — 사이에 저장소 계층이 들어가므로 그만큼 넓다 */
  childGapY: 126,
  /** 최하단 여백 (노드명 라벨 + 저장소 계층 공간 포함) */
  marginBottom: 96,
  /** 노드 반지름 — viewBox가 카드 폭에 맞춰 축소되므로 실제 렌더 크기보다 크게 잡는다 */
  serverRadius: 44,
  siloRadius: 24,
  childRadius: 18,
  /**
   * 노드 하단 저장소 계층.
   * 배치는 `원 → 노드명 → 저장소 1~3개 → 각 저장소의 샤드`.
   * 저장소 = MinIO 인스턴스(사각형), 샤드 = 그 안의 데이터 조각(작은 칸).
   */
  storage: {
    /** 노드명 베이스라인에서 저장소 사각형 윗변까지 (연결선이 보일 만큼 띄운다) */
    offsetY: 22,
    /** 저장소 사각형 크기 */
    unitWidth: 20,
    unitHeight: 13,
...
/** 토폴로지 화면 배율 조절 범위. 1 = 카드 폭에 꼭 맞춤 */
export const TOPOLOGY_ZOOM = {
  min: 0.6,
  max: 2.6,
  step: 0.2,
  default: 1,
